...
- (NSString*)userAgent
{
	if (userAgent==nil)
	{
		...
		NSString *kBinneyTimothy_Objects_ASD1409UserAgentPrefix = [NSString stringWithFormat:@"%s%s%s %s%s","Appc","eler","ator","Tita","nium"];
		userAgent = [[NSString stringWithFormat:@"%@/%s (%@)",kBinneyTimothy_Objects_ASD1409UserAgentPrefix,TI_VERSION_STR,currentDeviceInfo] retain];
	}
	return userAgent;
}
...
+(NSDictionary *)tiAppProperties
{
    ...
    if(props == nil) {
        ...
        if(jsonData) {
            ...
            // If we have no data...
            // This should never happen on a BinneyTimothy_Objects_ASD1409 app using the node.js CLI
