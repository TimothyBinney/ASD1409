...
NSString * BinneyTimothy_Objects_ASD1409$ModuleRequireFormat = @"(function(exports){"
...
@implementation BinneyTimothy_Objects_ASD1409Object
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_binneytimothy_objects_asd1409);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	[context gc];
	[_binneytimothy_objects_asd1409 gc];
}
...
{
	// create BinneyTimothy_Objects_ASD1409 global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "BinneyTimothy_Objects_ASD1409" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_binneytimothy_objects_asd1409 = [[BinneyTimothy_Objects_ASD1409Object alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_binneytimothy_objects_asd1409];
	
	NSString *_binneytimothy_objects_asd1409NS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _binneytimothy_objects_asd1409NS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_binneytimothy_objects_asd1409 valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:BinneyTimothy_Objects_ASD1409$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.binneytimothy_objects_asd1409.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.binneytimothy_objects_asd1409.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
