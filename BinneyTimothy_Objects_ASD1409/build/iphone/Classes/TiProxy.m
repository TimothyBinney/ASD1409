...
//Rare exceptions to indicate a bug in the _binneytimothy_objects_asd1409 code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_binneytimothy_objects_asd1409 = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_binneytimothy_objects_asd1409]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.binneytimothy_objects_asd1409.module"
