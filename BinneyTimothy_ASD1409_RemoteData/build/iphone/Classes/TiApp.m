...
- (NSString*)userAgent
{
	if (userAgent==nil)
	{
		...
		NSString *kBinneyTimothy_ASD1409_RemoteDataUserAgentPrefix = [NSString stringWithFormat:@"%s%s%s %s%s","Appc","eler","ator","Tita","nium"];
		userAgent = [[NSString stringWithFormat:@"%@/%s (%@)",kBinneyTimothy_ASD1409_RemoteDataUserAgentPrefix,TI_VERSION_STR,currentDeviceInfo] retain];
	}
	return userAgent;
}
...
+(NSDictionary *)tiAppProperties
{
    ...
    if(props == nil) {
        ...
        if(jsonData) {
            ...
            // If we have no data...
            // This should never happen on a BinneyTimothy_ASD1409_RemoteData app using the node.js CLI
