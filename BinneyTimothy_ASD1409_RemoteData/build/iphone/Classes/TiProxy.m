...
//Rare exceptions to indicate a bug in the _binneytimothy_asd1409_remotedata code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_binneytimothy_asd1409_remotedata = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_binneytimothy_asd1409_remotedata]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.binneytimothy_asd1409_remotedata.module"
