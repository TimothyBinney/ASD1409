...
NSString * BinneyTimothy_ASD1409_RemoteData$ModuleRequireFormat = @"(function(exports){"
...
@implementation BinneyTimothy_ASD1409_RemoteDataObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_binneytimothy_asd1409_remotedata);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	[context gc];
	[_binneytimothy_asd1409_remotedata gc];
}
...
{
	// create BinneyTimothy_ASD1409_RemoteData global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "BinneyTimothy_ASD1409_RemoteData" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_binneytimothy_asd1409_remotedata = [[BinneyTimothy_ASD1409_RemoteDataObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_binneytimothy_asd1409_remotedata];
	
	NSString *_binneytimothy_asd1409_remotedataNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _binneytimothy_asd1409_remotedataNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_binneytimothy_asd1409_remotedata valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:BinneyTimothy_ASD1409_RemoteData$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.binneytimothy_asd1409_remotedata.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.binneytimothy_asd1409_remotedata.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
